...
namespace KayitRehperi.Service.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<CustomResponseDto<TokenDto>> CreateTokenAsync(LoginDto loginDto)
        {
            ...
            if (user == null) return CustomResponseDto<TokenDto>.Fail("Email or Password is wrong", 400, true);

            if (!await _userManager.CheckPasswordAsync(user, loginDto.Password))
            {
                return CustomResponseDto<TokenDto>.Fail("Email or Password is wrong", 400, true);
            }
        ...
        public async Task<CustomResponseDto<NoContentDto>> RevokeRefreshToken(string refreshToken)
        {
            ...
            if (existRefreshToken == null)
            {
                return CustomResponseDto<NoContentDto>.Fail("Refresh token not found", 404, true);
            }
